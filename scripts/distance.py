...
def main(file1: str, file2: str, file3: str, file4: str, output_dir: str):
    ...
    distance1 = kl(file1, file2)
    distance2 = kl(file3, file4)
    distance3 = kl(file1, file3)

    distance1.to_csv(output_dir+"/base_sub1.tsv", sep="\t", header=False)
    distance2.to_csv(output_dir+"/distance.tsv", sep="\t", header=False)
    distance3.to_csv(output_dir+"/base_sub2.tsv", sep="\t", header=False)
    ...
    plt.title("Distance between two subsets, compared to base distance for each subset")
    plt.tight_layout()
